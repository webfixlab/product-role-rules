	...
	class Proler_Product_Settings {
		...
		public static function get_settings( $product ) {
            ...
            if( empty( $rs ) || -1 !== $rs ){ // empty or not disabled.
                $rs = self::global_settings( $id, $role );
            }
        ...
        private static function product_settings( $id, $role ) {
            ...
            return isset( $ps['roles'] ) && isset( $ps['roles'][$role] ) && !empty( $ps['roles'][$role] ) ? $ps['roles'][$role] : [];
        }
        private static function global_settings( $id, $role ) {
            ...
            return isset( $gs['roles'] ) && isset( $gs['roles'][$role] ) && !empty( $gs['roles'][$role] ) ? $gs['roles'][$role] : [];
        }
